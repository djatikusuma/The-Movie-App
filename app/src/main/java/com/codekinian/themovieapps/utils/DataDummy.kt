...
object DataDummy {
    fun generateDummyMovies(): Movie.MovieResult {
    ...
    fun generateDummyTvshows(): Tvshow.TvResult {
        return Tvshow.TvResult(
            listOf(
                Tvshow(
                    1,
                    "R.drawable.tv_arrow",
    ...
    fun getMovieById(id: Int): Movie? {
        for (movie in generateDummyMovies().results)
            if (id == movie.id) return movie
        return null
    }

    fun getTvById(id: Int): Tvshow? {
        for (tv in generateDummyTvshows().results)
